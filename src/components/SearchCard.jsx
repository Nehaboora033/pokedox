...
const SearchCard = ({ pokemon, description, evolution }) => {
  if (!pokemon) {   //empty no pokemon select 
    return (
      <div className='text-[20px] text-center text-gray-400  font-semibold flex items-center justify-center h-full'>
        <img src={pikachu} alt="pikachu" className=' w-[130px] absolute top-[-110px] mx-0 left-[30%]' />
  ...
  return (
    <div className='flex  w-full justify-center flex-col h-full relative'>
      <img
        ...
        alt={pokemon.name}
        className='pokemon-gif w-[100px] absolute -top-[80px] left-[40%]'
      />
        ...
        <div>
          ...
          <div className='flex gap-2 m-1'>
            {pokemon.abilities?.map(a => (
              <div
                key={a.ability.name}
                className='p-2 bg-[#f6f8fc] w-1/2 text-[14px] whitespace-nowrap rounded-full font-medium capitalize'
              >
                {a.ability.name}
              </div>
            ))}
